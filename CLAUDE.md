...
- This repo uses TypeScript project references; `tsc --noEmit` alone
  silently checks zero files and must never be used as a verification
  step. Use `tsc -b --noEmit` (build mode) or `npm run build` to get a
  real type-check. Any prior session claim of a "clean type-check" using
  the bare `tsc --noEmit` command should be treated as unverified, not
